...
export default function VideoList({
  ...
  useEffect(() => {
    ...
    if (searchText.trim()) {
      ...
      filtered = filtered.filter((v) => (v.title + v.author).toLowerCase().includes(q));
    }
  ...
  const handleShuffle = () => {
    ...
    if (searchText.trim()) {
      const q = searchText.toLowerCase();
      base = base.filter((v) => (v.title + v.author).toLowerCase().includes(q));
    }
    ...
    <main className="flex justify-center">
      <div className="w-full max-w-6xl px-4 py-6">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold leading-tight">楽曲一覧 ルーキー</h1>
        ...
        {!loading && displayData.length === 0 && (
          <div className="text-center py-20 text-gray-600">ルーキー楽曲はまだありません。</div>
        )}
        {/* グリッド */}
