...
def main():
    ...
    catalog_semifinal_sheet_config = config["vote_semifinal"][
    ...
    semifinal_group_size = (
        config["vote_grouping"]["group_num"] // 2
    )  # 準決勝グループ数=予選グループ数の半分

    df["グループID"] = df.apply(
        calc_semifinal_no, axis=1, args=(semifinal_group_size,)
    )  # 準決勝グループID列を作成し更新

    # シートに書き込み
    output_sheet = connect_sheet(
        catalog_semifinal_spreadsheetname, catalog_semifinal_sheetname
    )
    ...
    print(
        f"Successfully updated '{catalog_semifinal_sheetname}' in '{catalog_semifinal_spreadsheetname}' with grouped data."
    )
