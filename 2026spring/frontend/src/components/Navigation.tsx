...
import { Menu, Home, Info, Music, Vote, Star, Mail, Layout } from "lucide-react";
    ...
    <div className="drawer lg:drawer-open">
      ...
      <div className="drawer-side z-[100]">
        ...
        <ul className="menu p-4 w-72 min-h-full bg-base-100 text-slate-700 border-r border-mint/10 shadow-sm">
          ...
          <li className="menu-title mt-4 text-mint font-bold tracking-widest text-xs uppercase">詳細</li>
          <li><Link href="/details/about" onClick={closeDrawer}><Info className="w-4 h-4 mr-3 text-slate-400" /> イベントについて</Link></li>
          <li><Link href="/details/rookie" onClick={closeDrawer}><Layout className="w-4 h-4 mr-3 text-slate-400" /> ルーキー参加方法</Link></li>
          <li><Link href="/details/extra" onClick={closeDrawer}><Layout className="w-4 h-4 mr-3 text-slate-400" /> エクストラ参加方法</Link></li>
          ...
          <li className="menu-title mt-4 text-slate-500 font-bold tracking-widest text-xs uppercase">その他</li>
          <li><Link href="/contact" onClick={closeDrawer}><Mail className="w-4 h-4 mr-3 text-slate-400" /> 問い合わせ</Link></li>
        </ul>
      </div>
    </div>
  );
