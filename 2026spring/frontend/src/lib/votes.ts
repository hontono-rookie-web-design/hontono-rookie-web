...
export async function getPreliminaryForms() {
  const items = await fetchVotesSheet(CONFIG.voteformssheets.preliminaries.name);
  return items
    .filter((f) => f.formUrl)
    .map((f) => ({
      group: f.group,
...
export async function getPreliminaryRanks() {
  const items = await fetchRankingSheet(CONFIG.rankingsheets.preliminaries.name);
  return items.map((r) => ({
    videoId: r.videoId,
    group: r.group,
    rank: r.rank,
...
export async function getSemifinalForms() {
  const items = await fetchVotesSheet(CONFIG.voteformssheets.semifinals.name);
  return items
    .filter((f) => f.formUrl)
    .map((f) => ({
      group: f.group,
...
export async function getSemifinalRanks() {
  const items = await fetchRankingSheet(CONFIG.rankingsheets.semifinals.name);
  return items.map((r) => ({
    videoId: r.videoId,
    group: r.group,
    rank: r.rank,
...
export async function getFinalForms() {
  const items = await fetchVotesSheet(CONFIG.voteformssheets.finals.name);
...
export async function getFinalRanks() {
  const items = await fetchRankingSheet(CONFIG.rankingsheets.finals.name);
...
export async function getExForms() {
  const items = await fetchVotesSheet(CONFIG.voteformssheets.ex.name);
...
export async function getExRanks() {
  const items = await fetchRankingSheet(CONFIG.rankingsheets.ex.name);
